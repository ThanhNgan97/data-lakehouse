# -*- coding: utf-8 -*-
"""
clean_file_nguon_iceberg.py
Dọn dẹp tiền tố UUID (32 hex) trong cột file_nguon của bảng Silver và cập nhật lại Gold Data Marts.
"""
import os
import sys
...
def get_spark():
    return SparkSession.builder \
        .appName("Clean_File_Nguon_Iceberg") \
...
def main():
    ...
    try:
        ...
        if spark.catalog.tableExists("lakehouse.silver.kpi_cusc_master"):
            print("🧹 Đang dọn dẹp giá trị file_nguon trong bảng Silver lakehouse.silver.kpi_cusc_master...")
            df_silver = spark.read.table("lakehouse.silver.kpi_cusc_master")
            df_cleaned = df_silver.withColumn("file_nguon", regexp_replace(col("file_nguon"), r"^[a-f0-9]{32}_", ""))
            
            # Ghi đè lại bảng Silver
            df_cleaned.writeTo("lakehouse.silver.kpi_cusc_master").createOrReplace()
            print("✅ Đã dọn dẹp xong cột file_nguon trong bảng Silver.")
            
            print("🚀 Tự động chạy lại Silver -> Gold Data Marts...")
            run_silver_to_gold(spark)
            print("🎉 Hoàn tất dọn dẹp và rebuild tầng Gold!")
        else:
            print("ℹ️ Bảng Silver chưa tồn tại.")
    finally:
        spark.stop()
