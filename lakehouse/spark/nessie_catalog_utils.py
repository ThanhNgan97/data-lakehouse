...
def check_quality_api_silver(spark, table_name: str):
    """
    Kiểm tra chất lượng dữ liệu tối thiểu cho các bảng API Silver TRÊN BRANCH hiện tại:
      1. Bảng phải có ít nhất 1 dòng dữ liệu (không rỗng)
      2. Không có bản ghi thiếu các khóa chính cốt lõi (ví dụ: don_vi_dao_tao, ky_danh_gia)
      3. Không có checksum_sha256 bị trùng lặp
    """
    print(f"🔍 Đang kiểm tra chất lượng dữ liệu API Silver cho bảng {table_name}...")
    df = spark.table(table_name)

    total_rows = df.count()
    if total_rows == 0:
        raise DataQualityError(f"Bảng Silver API {table_name} rỗng, không có dữ liệu để merge.")

    # Kiểm tra null các cột khóa chính API (tùy thuộc vào bảng teaching hay learning)
    # Ở đây ta kiểm tra các cột chung không được phép NULL
    columns_to_check = [col for col in ["don_vi_dao_tao", "ky_danh_gia", "ma_sinh_vien"] if col in df.columns]
    if columns_to_check:
        null_condition = " OR ".join([f"{c} IS NULL" for c in columns_to_check])
        null_key_rows = df.filter(null_condition).count()
        if null_key_rows > 0:
            raise DataQualityError(
                f"Phát hiện {null_key_rows} bản ghi thiếu thông tin khóa chính ({', '.join(columns_to_check)})."
            )

    # Kiểm tra trùng lặp checksum nếu có cột checksum_sha256
    if "checksum_sha256" in df.columns:
        total_checksum = df.select("checksum_sha256").count()
        distinct_checksum = df.select("checksum_sha256").distinct().count()
        if total_checksum != distinct_checksum:
            dup_count = total_checksum - distinct_checksum
            raise DataQualityError(
                f"Phát hiện {dup_count} bản ghi có checksum_sha256 bị trùng lặp trong bảng API."
            )

    print(f"✅ Dữ liệu API Silver đạt chất lượng: {total_rows} dòng.")
    return True