...
def run_bronze_to_silver(spark, run_id=""):
    ...
    try:
        ...
        #     col("quy_danh_gia").isNotNull() & col("quy_danh_gia").rlike(r"^(Q[1-4]/20\d{2}|NĂM 20\d{2})$") &
        ...
        #     .when(col("quy_danh_gia").isNull() | ~col("quy_danh_gia").rlike(r"^(Q[1-4]/20\d{2}|NĂM 20\d{2})$"), "Missing or invalid quy_danh_gia format (must be Qn/YYYY or NĂM YYYY)")
        ...
        valid_condition = (
            col("ma_chi_tieu").isNotNull() & (col("ma_chi_tieu") != "") & (upper(col("ma_chi_tieu")) != "N/A") &
            col("quy_danh_gia").isNotNull() & col("quy_danh_gia").rlike(r"^\s*(Q[1-4]/20\d{2}|NĂM 20\d{2})\s*$") &
            col("ket_qua_he_thong").isNotNull() & (col("ket_qua_he_thong") != "")
        )
        
        # SỬA LỖI HỐ ĐEN NULL Ở ĐÂY:
        invalid_condition = (~valid_condition) | valid_condition.isNull()
        
        df_valid = df_staging_all.filter(valid_condition)
        df_invalid = df_staging_all.filter(invalid_condition).withColumn(
            "reject_reason", 
            when(col("ma_chi_tieu").isNull() | (col("ma_chi_tieu") == "") | (upper(col("ma_chi_tieu")) == "N/A"), "Missing or invalid ma_chi_tieu")
            .when(col("quy_danh_gia").isNull() | ~col("quy_danh_gia").rlike(r"^\s*(Q[1-4]/20\d{2}|NĂM 20\d{2})\s*$"), "Missing or invalid quy_danh_gia format (must be Qn/YYYY or NĂM YYYY)")
            .when(col("ket_qua_he_thong").isNull() | (col("ket_qua_he_thong") == ""), "Missing ket_qua_he_thong")
            .otherwise("Invalid data")
        )
