...
const PIPELINE_TASKS = [
  { id: 'ingest_bronze', label: 'Extract', sub: 'Bronze', tone: 'bronze', dot: 'bg-bronze-500' },
  { id: 'bronze_to_silver', label: 'Transform', sub: 'Silver', tone: 'silver', dot: 'bg-silver-500' },
  { id: 'silver_to_gold', label: 'Load', sub: 'Gold', tone: 'gold', dot: 'bg-gold-500' },
  { id: 'predictive_analysis', label: 'Analyze', sub: 'Predict', tone: 'lake', dot: 'bg-lake-500' },
];
...
const UserUpload = () => {
  ...
  const getStatus = (item) =>
    STATUS_CONFIG[item.pipeline_status] || { label: item.pipeline_status, tone: 'ink' };

  return (
    <div className="min-h-screen bg-[#F5F6FA] font-sans text-ink-900 flex flex-col">
      <header className="bg-white/85 backdrop-blur border-b border-ink-100 px-6 md:px-8 py-3.5 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <LakehouseMark className="w-10 h-10" />
          <div>
            <h1 className="text-base md:text-lg font-bold text-ink-900 leading-tight">EduLakehouse Portal</h1>
            <p className="text-[11px] md:text-xs text-ink-400 font-data">Cổng nạp & xử lý dữ liệu trung tâm</p>
          </div>
        </div>
        <button
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <Card className="p-6">
            <div className="mb-5">
              ...
              <h2 className="text-lg font-bold text-ink-900">Tải lên Dữ liệu</h2>
              <p className="text-xs text-ink-400 mt-1">Hệ thống sẽ tự động đưa vào Bronze Zone và kích hoạt Pipeline</p>
            </div>
            ...
            <div
              ...
              className={`relative overflow-hidden border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-300 flex flex-col items-center justify-center group ${
                uploading ? 'cursor-wait opacity-90 bg-ink-50 border-ink-200' : 'cursor-pointer border-lake-200 hover:border-lake-400 hover:bg-lake-50/50'
              } ${isDragging ? 'border-lake-500 bg-lake-50 scale-[1.02]' : ''}`}
            >
              <div className={`w-14 h-14 mb-3 rounded-xl flex items-center justify-center transition-all duration-300 ${
                isDragging ? 'bg-lake-500 text-white scale-110' : 'bg-lake-50 text-lake-600 group-hover:scale-110 group-hover:bg-lake-500 group-hover:text-white'
              }`}>
                <Icon name={uploading ? 'clock' : 'uploadCloud'} className="w-6 h-6" />
              </div>
              <p className="text-sm font-semibold text-ink-800">
                {uploading ? 'Đang tải lên và xử lý...' : 'Kéo thả file hoặc bấm để chọn'}
              </p>
              <p className="text-[11px] text-ink-400 mt-1.5 font-data">Hỗ trợ định dạng: PDF, DOCX</p>

              {uploading && uploadProgress > 0 && (
                <div className="absolute bottom-0 left-0 h-1.5 bg-lake-500 transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
              )}
            </div>
            ...
            <Card className="p-6 flex-1">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <h3 className="text-base font-bold text-ink-900">Tiến trình Pipeline</h3>
                  <p className="text-xs text-ink-400 mt-0.5">Airflow Orchestration</p>
                </div>
              ...
              <div className="relative pl-5 border-l-2 border-ink-100 space-y-6 ml-2">
                {PIPELINE_TASKS.map((pt) => {
                  ...
                  if (state === 'success') {
                    ...
                    stateText = 'Thành công'; icon = 'checkCircle'; textColor = 'text-emerald-700'; cardBorder = 'border-emerald-100';
                  } else if (state === 'running') {
                    dotClass = `${pt.dot} border-white animate-pulse shadow-[0_0_0_3px_rgba(15,151,168,0.2)]`;
                    stateText = 'Đang xử lý...'; icon = 'activity'; textColor = 'text-lake-700'; cardBorder = 'border-lake-200'; cardBg = 'bg-lake-50/50';
                  } else if (state === 'failed') {
                    dotClass = 'bg-rose-500 border-white shadow-[0_0_0_3px_rgba(244,63,94,0.15)]';
                    stateText = 'Lỗi'; icon = 'alertTriangle'; textColor = 'text-rose-700'; cardBorder = 'border-rose-200';
                  }

                  return (
                    <div key={pt.id} className="relative">
                      <div className={`absolute -left-[27px] top-2 w-3.5 h-3.5 rounded-full border-2 ${dotClass} z-10 transition-colors duration-300`} />
                      <div className={`rounded-xl p-3 border ${cardBorder} ${cardBg} transition-all duration-300`}>
                        <div className="flex items-center justify-between">
                          <div>
                            <p className={`font-semibold text-sm ${state === 'pending' ? 'text-ink-400' : 'text-ink-800'}`}>
                              {pt.label} <span className="text-ink-300 font-data text-[11px] font-normal">· {pt.sub}</span>
                            </p>
                          </div>
                          <Icon name={icon} className={`w-4 h-4 ${textColor}`} />
                        </div>
                        <p className={`text-[11px] font-data font-medium mt-1 ${textColor}`}>{stateText}</p>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="lg:col-span-8 flex flex-col gap-6 h-full">
          ...
          <Card className="overflow-hidden flex flex-col h-[320px]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-ink-100 bg-white shrink-0">
              <div>
                <h3 className="font-bold text-ink-900 text-base">Lịch sử tải lên</h3>
                <p className="text-[11px] text-ink-400 mt-0.5">Tự động đồng bộ trạng thái Pipeline mỗi 5 giây</p>
              </div>
              <button
              ...
              >
                <Icon name="refresh" className={`w-3.5 h-3.5 ${historyLoading ? 'animate-spin' : ''}`} />
            ...
            <div className="flex-1 overflow-auto bg-[#FAFBFD] p-4">
              {history.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-ink-300">
                  ...
                  <p className="text-sm font-medium text-ink-400">Chưa có dữ liệu nào được tải lên hệ thống.</p>
                </div>
              ) : (
                      ...
                      >
                        <div className="flex items-center gap-3.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h4 className="font-semibold text-ink-800 text-sm truncate" title={item.filename}>{item.filename}</h4>
                            <div className="text-[11px] text-ink-400 mt-1 flex items-center gap-2 truncate font-data">
                              <span className="font-medium text-ink-500">{item.username}</span>
                              <span className="text-ink-200">•</span>
                              <span title={item.dag_run_id}>{item.dag_run_id?.slice(0, 12)}...</span>
                            </div>
                          </div>
                        </div>
          ...
          <Card className="overflow-hidden flex flex-col flex-1 min-h-[450px]">
            <div className="px-6 py-4 border-b border-ink-100 bg-white flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-base font-bold text-ink-900">Báo cáo Phân tích</h3>
                <p className="text-[11px] text-ink-400 mt-0.5 font-data">Gold Layer · Apache Superset</p>
              </div>
              <a
                href={supersetUrl}
                target="_blank"
