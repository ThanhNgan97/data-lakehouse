...
def main():
    ...
    try:
        use_main(spark)
        print("Searching Quarantine table for danh_gia_muc_tieu_kpi...")
        if spark.catalog.tableExists("lakehouse.silver.kpi_cusc_quarantine"):
            df_q = spark.read.table("lakehouse.silver.kpi_cusc_quarantine").filter("file_nguon LIKE '%danh_gia_muc_tieu_kpi%'")
            print(f"Quarantine matching count: {df_q.count()}")
            df_q.select("file_nguon", "ma_chi_tieu", "quy_danh_gia", "ket_qua_he_thong", "reject_reason").show(50, truncate=False)
        else:
